...
public class HillClimbingAgent extends Agent {
    ...
    {
    	ArrayList<Coordinate> children = getAndCheckNeighbors(pt,goalPt,xExtent,yExtent,obstacles);
    	// for each child, find the child with the best utility (lowest heur)
    	float minUtil = Float.POSITIVE_INFINITY;
    	Coordinate nextPt = children.get(0);
    	for (int k = 0; k <= children.size()-1; k++) {
    		float util = heuristic(children.get(k).x,children.get(k).y,goalPt);
    		if (util < minUtil) {
    			nextPt = children.get(k);
    			minUtil = util;
    		}
    	}
    	float currUtil = heuristic(pt.x,pt.y,goalPt);
    	if (minUtil < currUtil) {
    		return nextPt;
    	}
    	return pt;
